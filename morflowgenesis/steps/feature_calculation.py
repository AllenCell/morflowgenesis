...
def get_valid_features(features):
    """Parse passed features and return only valid ones."""
...
def get_matched_roi_features(img, features, channels, reference):
    """Calculate features for a matched pair of images (e.g. F1 score between two segmentations)"""
    features = get_valid_features(features)
    img = ensure_channel_first(img)
...
def get_matched_cell_features(row, features, channels, reference_channel):
    """Calculate features for a matched pair of images (e.g. spherical harmonics using shared
...
def get_objects(object, input_step, reference_step):
    """Create iterable of objects to parallelize feature calculation over."""
...
def create_output(image_object, output_name, features):
    ...
    step_output.save(pd.concat(features))
...
def process_object(
    ...
    create_output(image_object, output_name, features)
