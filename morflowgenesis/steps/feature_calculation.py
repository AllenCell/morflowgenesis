...
def calculate_features(image_object, step_name, output_name,  input_step, features,segmentation_columns, matching_step = None, reference_step = None):
    ...
    for row in cell_df.itertuples():
        ...
        if reference_step is None:
            ...
            results.append(get_matched_features(row, row_label, features, segmentation_columns)) #.submit
    # features_df = pd.concat([r.result() for r in results])
    features_df = pd.concat(results)

    output = StepOutput(image_object.working_dir,step_name, output_name, output_type = 'csv', image_id = image_object.id)
