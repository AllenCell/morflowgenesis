...
from morflowgenesis.utils import create_task_runner, ImageObject, StepOutput
...
def create_mesh(image_object,step_name, output_name, seg_step, resize):
    ...
    mesh, _, _ = shtools.get_mesh_from_image(
        seg, sigma=0, lcc=False, translate_to_origin=False
    )

    mesh_output_directory = (
        image_object.working_dir / "mesh" / output_name / seg_step
    )

    # rename to make movie creation easy
    timepoint = image_object.metadata.get('T')
    save_path = mesh_output_directory / f"{image_object.source_path}_T{timepoint:04d}.vtk" if timepoint is not None else mech_output_directory/f"{image_object.id}.vtk"
