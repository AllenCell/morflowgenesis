...
@task
def align(
    image_object, image_step, segmentation_step, boundary=False,
):
    img = image_object.load_step(image_step)
    seg = image_object.load_step(segmentation_step)>0
...
@task
def run_object(
    image_object, image_step, segmentation_steps, run_within_object, boundary=False,
):
    results = []
