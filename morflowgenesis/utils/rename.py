...
def main(base_dir, steps={}, inplace=False):
    ...
    image_objects = [
        ...
        for fn in tqdm.tqdm(
            (basedir / "_ImageObjectStore/").glob("*"), desc="Loading image objects"
        )
    ]

    name_map = {
        image_object.id: os.path.basename(image_object.source_path)
    ...
    for step_name, output_name in steps.items():
        for fn in (basedir / step_name / output_name).glob("*"):
            ...
            if inplace:
                try:
                    os.rename(
                        fn, basedir / step_name / f"{output_name}_renamed" / name_map[fn.stem]
                    )
                except:
                    continue
            else:
